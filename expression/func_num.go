...
func (this *Round) Apply(context Context, args ...value.Value) (value.Value, error) {
	...
	if len(this.operands) == 1 {
		return value.NewValue(roundFloat(v, 0, true)), nil
	}
...
///////////////////////////////////////////////////
//
// RoundNearest - away from zero
//
///////////////////////////////////////////////////

/*
Same as ROUND but to nearest value away from zero.
*/
...
func (this *RoundNearest) Apply(context Context, args ...value.Value) (value.Value, error) {
	arg := args[0]
	if arg.Type() == value.MISSING {
		return value.MISSING_VALUE, nil
	} else if arg.Type() != value.NUMBER {
	...
	if len(this.operands) == 1 {
		return value.NewValue(roundFloat(v, 0, false)), nil
	}
