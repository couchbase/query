...
value.go: The first constant declaration is _Tristate_.  It has three states, _TRUE_, _FALSE_ and _NONE_ (initialized to _iota_, which is used in constant declarations in Go. Its value starts at 0 in the const block and increments each time it is seen.  Refer to the Golang docs). Value supports different data types that are supported by N1ql. They are Missing, Null, Binary, Boolean, Number, String, Array, Object and JSON. Marshal returns the JSON encoding of any input interface. It is used while implementing the method MarshalJSON. The type ValueChannel is a channel of Value, where Value is an interface. (Channels in Go are pipes that connect concurrent goroutines) The results in a request for the server are of type ValueChannel. Values is a collection of Value. The type is defined as a slice of Value. Similarly CompositeValues is a slice of Values.
