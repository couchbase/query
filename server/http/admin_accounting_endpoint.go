	...
	if req.Method == "DELETE" {
		...
		prepareds.PreparedDo(name, func(entry *prepareds.CacheEntry) {
			...
			if entry.Prepared.OptimHints() != nil {
				hints, _ := json.Marshal(entry.Prepared.OptimHints())
				itemMap["optimizer_hints"] = value.NewParsedValue(hints, true)
			}
	...
	case "GET":
		...
		snapshot := func(name string, d *prepareds.CacheEntry) bool {
			...
			if d.Prepared.OptimHints() != nil {
				hints, _ := json.Marshal(d.Prepared.OptimHints())
				data[i]["optimizer_hints"] = value.NewParsedValue(hints, true)
			}
...
func doActiveRequest(endpoint *HttpEndpoint, w http.ResponseWriter, req *http.Request, af *audit.ApiAuditFields) (
	...
	if req.Method == "GET" || req.Method == "POST" {
		...
		return processActiveRequest(endpoint, requestId, (req.Method == "POST")), nil
...
func processActiveRequest(endpoint *HttpEndpoint, requestId string, profiling bool) interface{} {
	var res interface{}
...
func doActiveRequests(endpoint *HttpEndpoint, w http.ResponseWriter, req *http.Request, af *audit.ApiAuditFields) (
	...
	requests := make([]interface{}, numRequests)
...
func CaptureActiveRequests(endpoint *HttpEndpoint, w io.Writer) error {
	...
	endpoint.actives.ForEach(func(requestId string, request server.Request) bool {
		...
		if err == nil {
			r := activeRequestWorkHorse(endpoint, request, true)
...
func doCompletedRequest(endpoint *HttpEndpoint, w http.ResponseWriter, req *http.Request, af *audit.ApiAuditFields) (
	...
	if req.Method == "GET" || req.Method == "POST" {
		...
		return processCompletedRequest(requestId, (req.Method == "POST")), nil
	} else if req.Method == "DELETE" {
...
func processCompletedRequest(requestId string, profiling bool) interface{} {
	var res interface{}
...
func CaptureCompletedRequests(w io.Writer) error {
	...
	server.RequestsForeach(func(requestId string, request *server.RequestLogEntry) bool {
		...
		if err == nil {
			r := completedRequestWorkHorse(request, true)
			err = json.MarshalNoEscapeToBuffer(r, &buf)
			if err == nil {
				_, err = buf.WriteTo(w)
				buf.Reset()
