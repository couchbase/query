...
The most common form of the FROM clause is specifying a single identifier which identifies a bucket name.  In this form, an alias can optionally be specified with the AS clause.  When spcified this alias is the identifier used to explicitly refer to this data source in the query.  If omitted, the data source is referred to using the bucket name itself.
...
If the DISTINCT keyword was specified, duplicate result objects are removed from the result array.  If DISTINCT was not specified, the statement returns all qualifying objects.
...
Arithemetic terms allow for performing basic arithmetic within an expression.  The standard addition, subtraction, multiplication, division, and modulo operators are supported.  DIV performs integer (instead of floating-point) division.  Additionally, a negation operator will change the sign of the expression.
...
If both operands are strings, the `||` operator concatenates these strings.  If either operand is an array, the result is an array containing the elements of both operands.  Otherwise the expression evaluates to NULL.
...
Subquery expressions return the result of evaluating a subquery.  When a subquery returning a single field is compared against a non-object value, the value of the subquery field, instead of the subquery's result object, is used in the comparison.
...
The existential expression, also known as the existential quantifier, evaluates to TRUE if and only if the subquery returns at least one row.  Otherwise the existential expression evaluates to FALSE.

in-expr:

![](diagram/in-expr.png)

IN expressions allow you test whether or not the left-hand side operand is equal to one of the values returned by the right-hand size collection.  The right-hand side must evaluate to an array or subquery.  Otherwise, the IN expression evaluates to NULL.

collection-predicate:

![](diagram/collection-predicate.png)

Collection predicates allow you to test boolean expressions against collections.  Two variants are supported: ANY and ALL.

First, the input collections is deteremined by the right-hand side expression in the  **OVER** clause.  Elements of the collection are iterated through, and assigned the `identifier` in the optional **AS** clause.  This identifier is only valid while evalutating the expression in the **ANY** or **ALL** clause.
...
For the following examples consider a bucket containing the following document with ID "n1ql-2013"
...
The [grammar](https://github.com/couchbaselabs/query/blob/master/docs/n1ql-select.ebnf) forming the basis of this document is written in a [W3C dialect of EBNF](http://www.w3.org/TR/REC-xml/#sec-notation).
...
1.  Document handling of NaN and +/- infinity as these may arise during expression evaluation and cannot be represented in standard JSON.

1.  EXISTS as currently defined returns TRUE for MISSING fields (which return rows with empty objects).
