...
type DropCatalog struct {
	base
	plan *plan.DropCatalog
}

func NewDropCatalog(plan *plan.DropCatalog, context *Context) *DropCatalog {
	rv := &DropCatalog{
		plan: plan,
	}
...
func (this *DropCatalog) Copy() Operator {
	rv := &DropCatalog{plan: this.plan}
	this.base.copy(&rv.base)
	return rv
}

func (this *DropCatalog) PlanOp() plan.Operator {
	return this.plan
}

func (this *DropCatalog) RunOnce(context *Context, parent value.Value) {
	this.once.Do(func() {
		...
		err := cbDs.DropCatalog(context, node.Name())
		if err != nil {
			nfe := errors.IsNotFoundError("Catalog", err)
