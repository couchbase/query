...
func (this *builder) VisitSubselect(node *algebra.Subselect) (interface{}, error) {
	...
	if this.order != nil {
		...
		if hasVector || hasProjAlias || hasLetVariable {
			...
			// Without a LIMIT there's no bound justifying the ANN index's inherently
	...
	if this.countScan == nil {
		...
		if projection.Distinct() || this.setOpDistinct {
			if this.useCBO && this.lastOp != nil {
				...
				frCost = this.lastOp.FrCost()
				if cost > 0.0 && cardinality > 0.0 && size > 0 && frCost > 0.0 {
					cost, cardinality, size, frCost = getDistinctCost(projection.Terms(), cost, cardinality, size, frCost,
						this.keyspaceNames)
				}
			}
