	...
	if nSargKeys > 1 {
		...
		for _, filter := range baseKeyspace.Filters() {
			...
			if nsarg > 1 {
				...
				// we can allow using of individual filters for sarging of indexes
				// in the first case but not in the second case
				// this does leave
				//   (c1 = 1 AND c2 = 2) OR (c1 = 1 AND c2 = 3)
				// which is equivalent to the first one also not able to use
				// individual filters tos sarg, since we don't detect actual
				// duplicated filter in subarms of OR currently.
				return false
			}
		}
	}
