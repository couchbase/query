...
func (this *builder) buildIndexFilters(entry *indexEntry, baseKeyspace *base.BaseKeyspace,
	...
	var indexFilters, sortExprs expression.Expressions
	...
	if len(indexFilters) > 0 || len(sortExprs) > 0 {
		keys := entry.keys
...
func (this *builder) orSargUseFilters(pred *expression.Or, baseKeyspace *base.BaseKeyspace,
	...
	for i, child := range pred.Operands() {
		cmin, cmax, _, cskeys := SargableFor(child, entry.sargKeys, missing, skip, this.context, this.aliases)
		if i == 0 {
			min = cmin
			max = cmax
			skeys = cskeys
