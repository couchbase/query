...
func (this *builder) matchUnnest(node *algebra.KeyspaceTerm, pred, subset expression.Expression,
	...
	*indexEntry, *algebra.Unnest, *expression.All, error) {
	...
	min, max, sum, include, skeys := SargableFor(pred, vpred, entry.index, keys, entry.includes,
		missing, skip, isArrays, this.context, this.aliases)
	...
	if useCBO {
		cost, selectivity, cardinality, size, frCost, _ =
			indexScanCost(entry, origKeys, entry.sargIncludes, this.context.RequestId(),
				spans, includeSpans, node.Alias(), baseKeyspace.Keyspace(),
				-1, -1, this.advisorValidate(), this.context)
