	...
	*UnifiedDocumentRetriever, errors.Error) {
	...
	if flags&NO_RANDOM_SCAN == 0 {
		...
		if c, ok := context.(interface{ IsFeatureEnabled(uint64) bool }); ok {
			if !c.IsFeatureEnabled(util.N1QL_RANDOM_SCAN) {
	...
	logging.Debugf("%s", udr.Name(), context)
	...
	logging.Debugf("sampleSize: %v", udr.sampleSize, context)
		...
		if ok {
			...
			if udr.isFlagOn(SAMPLE_ALL_DOCS) {
				udr.rs_scan, err = udr.rs.StartRandomScan(context, math.MaxInt, 0,
					int(datastore.GetScanCap()), tenant.IsServerless(), udr.isFlagOn(INCLUDE_XATTRS), true)
			} else {
				udr.rs_scan, err = udr.rs.StartRandomScan(context, udr.sampleSize, 0,
					int(datastore.GetScanCap()), tenant.IsServerless(), udr.isFlagOn(INCLUDE_XATTRS), true)
			}
			if err != nil {
				logging.Debugf("random scan start failed: %v", err, context)
	...
	if udr.isFlagOff(NO_RANDOM_ENTRY) {
		...
		if ok {
			...
			for i = 0; i < _KEYS_NOT_FOUND; i++ {
				_, val, _ := udr.rnd.GetRandomEntry(udr.isFlagOn(INCLUDE_XATTRS), context)
				if val != nil {
					break
				}
			}
	...
	if udr.isFlagOff(NO_PRIMARY_INDEX) || udr.isFlagOff(NO_SECONDARY_INDEX) {
		...
		if err == nil {
			...
			if udr.isFlagOff(NO_PRIMARY_INDEX) {
				...
				if err == nil {
					...
					for _, index := range primaryIndexes {
						...
						found = true
						logging.Debugf("primary index (%v) found", index.Name(), context)
					...
					for _, idx := range ilist {
						if state, _, err := idx.State(); err == nil && state == datastore.ONLINE && !idx.IsPrimary() {
							if i3, ok := idx.(datastore.Index3); ok {
								...
								if udr.isFlagOff(ALLOW_SUPERSET_CONDITIONS) && i3.Condition() != nil {
									for n, other := range udr.indexes {
										...
										if plannerbase.SubsetOf(i3.Condition(), other.Condition()) {
											logging.Debugf("excluding %v - subset of %v", i3.Name(), other.Name(), context)
											continue secondary_indexes
										} else if plannerbase.SubsetOf(other.Condition(), i3.Condition()) {
											logging.Debugf("swapping secondary index %v for %v", i3.Name(), other.Name(), context)
											udr.indexes[n] = i3
											continue secondary_indexes
										}
									}
								...
								logging.Debugf("secondary index %v included", idx.Name(), context)
								methods++
								found = true
				...
				if udr.isFlagOff(SINGLE_INDEX) && len(udr.indexes) > 1 {
					...
					if udr.isFlagOff(ALLOW_DUPLICATED_LEADING_KEY) {
						...
						for i := range udr.indexes {
							...
							if leading[lkey] {
								logging.Debugf("%v excluded - duplicate leading key", udr.indexes[i].Name(), context)
								udr.indexes[i] = nil
								prune = true
							} else {
								leading[lkey] = true
			...
			logging.Debugf("rs: %v rnd: %v idxs: %v", udr.rs != nil, udr.rnd != nil, len(udr.indexes), context)
		} else {
		...
		if err == nil {
			...
			if err == nil {
				...
				for _, index := range seqScan {
					...
					methods++
					found = true
					logging.Debugf("sequential scan found for %v", ks.Name(), context)
					break
				}
			}
			if err != nil || !found {
...
func (udr *UnifiedDocumentRetriever) GetNextDoc(context datastore.QueryContext) (string, value.Value, errors.Error) {
	if udr.returned >= udr.sampleSize && (udr.rs == nil || udr.cacheActive ||
		...
		if udr.rs != nil && udr.rs_scan != nil {
			ru, err := udr.rs.StopScan(udr.rs_scan)
			if ru > 0 {
	...
	if udr.rs != nil {
		...
		k, v, e, cont := udr.getNextRandomScanDoc(context)
		if v != nil || e != nil || !cont {
			return k, v, e
		}
		logging.Debugf("random scan exhausted", context)
		udr.rs = nil
	}
	...
	if udr.rnd != nil && udr.isFlagOff(RANDOM_ENTRY_LAST) {
		...
		logging.Debugf("random retriever exhausted", context)
		udr.rnd = nil
	}
	...
	if len(udr.indexes) > 0 {
		...
		logging.Debugf("indexes exhausted", context)
	}
	...
	if udr.ss != nil {
		...
		logging.Debugf("full scan exhausted", context)
	}
...
func (udr *UnifiedDocumentRetriever) getRandom(context datastore.QueryContext) (string, value.Value, errors.Error) {
	...
	for duplicates < _MAX_DUPLICATES {
		key, value, err := udr.rnd.GetRandomEntry(udr.isFlagOn(INCLUDE_XATTRS), context)
		if err != nil {
			logging.Debugf("random retriever error: %v", err, context)
...
func (udr *UnifiedDocumentRetriever) getNextRandomScanDoc(context datastore.QueryContext) (
	string, value.Value, errors.Error, bool) {

	for {
		if len(udr.keys) == 0 {
			var err errors.Error
			var docs []value.AnnotatedValue
			docs, err, _ = udr.rs.FetchDocs(udr.rs_scan, _FETCH_TIMEOUT)
			if err != nil {
				if udr.returned == 0 && (udr.rnd != nil || len(udr.indexes) > 0 || udr.ss != nil) {
					logging.Debugf("random scan fetch failed (%v) trying other methods", err, context)
					udr.rs.StopScan(udr.rs_scan)
					udr.rs = nil
					udr.rs_scan = nil
			...
			if len(docs) == 0 {
				...
				udr.keys = make([]string, len(docs))
				udr.docs = make(map[string]value.AnnotatedValue, len(docs))
				for i := range docs {
					udr.keys[i] = docs[i].GetId().(string)
					if udr.isFlagOn(INCLUDE_XATTRS) {
						if docs[i].GetMetaField(value.META_XATTRS) == nil {
							// set the xattrs to MISSING so the inferencer doesn't consider it different just because of
							// the absence of xattrs
							docs[i].SetMetaField(value.META_XATTRS, value.MISSING_VALUE)
						}
					}
					udr.docs[udr.keys[i]] = docs[i]
				}
			}
			logging.Debugf("fetched %d documents from random scan", len(docs), context)
		}
		if len(udr.keys) == 0 {
			ru, err := udr.rs.StopScan(udr.rs_scan)
			if ru > 0 {
			...
			if udr.returned < udr.sampleSize {
				logging.Debugf("random scan stopped short (%v) of sample size (%v)", udr.returned, udr.sampleSize, context)
				udr.rs = nil
				return _EMPTY_KEY, nil, err, true
			}
			return _EMPTY_KEY, nil, err, false
...
func (udr *UnifiedDocumentRetriever) getNextFullScan(context datastore.QueryContext) (string, value.Value, errors.Error, bool) {
	if len(udr.keys) == 0 {
		if udr.iconn == nil {
			...
			logging.Debugf("scanning %v (scan: %v)", udr.ss.Name(), udr.scanNum, context)
		}
		...
		if udr.iconn == nil && len(udr.keys) == 0 {
			...
			if udr.currentIndex >= len(udr.indexes) || (udr.currentIndex > 0 && udr.isFlagOn(SINGLE_INDEX)) {
				...
				logging.Debuga(func() string {
					...
					return fmt.Sprintf("ending index scanning after %v index(es), %v docs returned", ci, udr.returned)
				}, context)
				return _EMPTY_KEY, nil, nil, true
			}
			...
			if udr.scanNum > 0 {
				...
				logging.Debugf("%v: ss-size: %v, sb-size: %v, remaining samples: %v",
					udr.indexes[udr.currentIndex].Name(), udr.scanSampleSize, udr.scanBlockSize, remainingSampleSize, context)
			}
...
func (kvrdr *KVRandomDocumentRetriever) GetNextDoc(context datastore.QueryContext) (string, value.Value, errors.Error) {
	...
	for duplicatesSeen < _MAX_DUPLICATES {
		resp, err := kvrdr.bucket.GetRandomDoc(false)
...
func MakeExpressionDocumentRetriever(context datastore.QueryContext, expr expression.Expression, sampleSize int) (
	*ExpressionDocumentRetriever, errors.Error) {
	...
	if sq, ok := expr.(*algebra.Subquery); ok {
		...
		edr.subquery, err = ectx.OpenStatement(sq.Select().String(), nil, nil, false, true, false, "")
		if err != nil {
