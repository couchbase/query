...
func TestInsertCaseFiles(t *testing.T) {
	fmt.Println("\n\nInserting values into Bucket for Where Functions \n\n ")
	qc := start()
...
func TestCleanupData(t *testing.T) {
	...
	_, _, errfs := Run_test(qc, "delete from product where test_id = \"where_func\"")
	if errfs != nil {
		t.Errorf("did not expect err %s", errfs.Error())
	}

	_, _, errfs = Run_test(qc, "delete from orders where test_id = \"where_func\"")
	if errfs != nil {
		t.Errorf("did not expect err %s", errfs.Error())
	}
}
