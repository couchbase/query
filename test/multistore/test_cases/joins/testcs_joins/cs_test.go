...
/*
Insert data into the product bucket created earlier
...
func TestCleanupData(t *testing.T) {
	...
	_, _, errcs := Run_test(qc, "delete from purchase where test_id = \"joins\"")
	if errcs != nil {
		t.Errorf("did not expect err %s", errcs.Error())
	}

	_, _, errcs = Run_test(qc, "delete from customer where test_id = \"joins\"")
	if errcs != nil {
		t.Errorf("did not expect err %s", errcs.Error())
	}

	_, _, errcs = Run_test(qc, "delete from product where test_id = \"joins\"")
	if errcs != nil {
		t.Errorf("did not expect err %s", errcs.Error())
	}
