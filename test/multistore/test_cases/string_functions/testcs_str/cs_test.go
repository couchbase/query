...
func TestInsertCaseFiles(t *testing.T) {
	fmt.Print("\n\nInserting values into Bucket for String Functions \n\n ")
	qc := Start_test()
...
func TestCleanupData(t *testing.T) {
	...
	_, _, errcs := Run_test(qc, "delete from orders where test_id = \"str_func\"")
	if errcs != nil {
		t.Errorf("did not expect err %s", errcs.Error())
	}
	_, _, errcs = Run_test(qc, "delete from customer where test_id = \"str_func\"")
	if errcs != nil {
		t.Errorf("did not expect err %s", errcs.Error())
	}
}
