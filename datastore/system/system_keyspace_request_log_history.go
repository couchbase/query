...
func (b *requestLogHistoryKeyspace) Fetch(keys []string, keysMap map[string]value.AnnotatedValue,
	...
	local := func(doc map[string]interface{}) bool {
		...
		localValue.SetMetaField(value.META_KEYSPACE, b.fullName)
		if ok {
			localValue.SetMetaField(value.META_PLAN, value.ApplyDurationStyleToValue(context.DurationStyle(), value.NewValue(t)))
		}
		if ook {
			localValue.SetMetaField(value.META_OPT_ESTIMATES, value.NewValue(o))
		}
