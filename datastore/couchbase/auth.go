...
func privilegeString(namespace, target, obj string, requested auth.Privilege) (string, error) {
	...
	case auth.PRIV_CATALOGS_WRITE:
		permission = "cluster.admin.catalogs!write"
	case auth.PRIV_CATALOG_SELECT:
		permission = join3Strings("cluster.catalog[", target, "].n1ql.select!execute")
	case auth.PRIV_CATALOG_UPDATE:
		permission = join3Strings("cluster.catalog[", target, "].n1ql.update!execute")
	case auth.PRIV_CATALOG_INSERT:
		permission = join3Strings("cluster.catalog[", target, "].n1ql.insert!execute")
	case auth.PRIV_CATALOG_DELETE:
		permission = join3Strings("cluster.catalog[", target, "].n1ql.delete!execute")
	default:
